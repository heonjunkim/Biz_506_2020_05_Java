...
import com.biz.bank.service.BankServiceV4;
...
public class BankEx_04 {
	...
	public static void main(String[] args) {
		...
		BankService bService = new BankServiceV4();
		...
		while(true) {
			...
			try {
				...
				System.out.println("메뉴 선택은 숫자로만 입력하세요");
				continue;
			 ...
			 * 		이 method를 외부에서 호출하여 어떤 행동을 수행해야 할 
			 * 		필요가 있을때가 있다.
			 * 
			 * 객체를 선언할때 interface로 선언을 하면 
			 * 		class에 구현된 method를 직접 호출하는 것이 안된다.
			 * 		이럴때 down Casting을 해서 해당 method를 사용할 수 있다.
			 */
			
			// interface에 설정되지 않은 V4에만 있는 메서드를 호출하는 방법
			// down Casting
			boolean bRet = ((BankServiceV4)bService).account();
			if( !bRet ) {
				continue;
			}
