...
public class AddrService01 {
	 ...
	 * addrList라는 필드 변수를 선언 "!만!" 해 둔 상태
	 * 선언만 된 필드 변수 중에 객체형 태의 변수는 
	 ...
	 */
	private List<AddrVO> addrList = new ArrayList<AddrVO>(); //   1.필드 변수를 선언과 동시에 초기화
	...
	// 		이후의코드에서 NullPointerException이 발생하는 것을 방지한다.
	...
	public void makeAddr() {
		...
		// AddrVO 클래스를 사용하여
		// 		adddrVO 객체(인스턴스)를 선언
		// 			생성자method를 호출하여 객체를 초기화(사용가능하도록)한다.
