...
public class Grade_01 {
	...
	public static void main(String[] args) {
		...
		// 각 학생의 총점, 평균을 계산하여 stdSum, stdAvg 에 저장
		for(int i = 0; i<stdCount; i++) {
			stdSum[i] = intKor[i];
			stdSum[i] += intEng[i];
			stdSum[i] += intMath[i];
		...
		System.out.println("---------------------------------------------------------------------------------------");
		System.out.println("학번\t국어\t영어\t수학\t음악\t예술\t총점\t평균");
		
		
		for(int i =0;i<stdCount; i++) {
			System.out.printf("%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",
					 (i+1),intKor[i], intEng[i], intMath[i], intMusic[i], intArt[i],
					 stdSum[i], stdAvg[i]);			
		}		
		...
		// printf에서 format 기호
		// %d : (Desimal)정수, 십진수 등을 표현하는 기호
		// %s : (String)문자열을 표현하는 기호
		// %c : (Charactor)문자를 표현하는 기호
		
		// \t : tab(8칸) 만큼 빈칸을 만들어라
		// \n : Enter키를 누른것처럼 줄바꿈을 하여라
