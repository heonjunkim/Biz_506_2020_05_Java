 ...
 * 
 * static 클래스는 초기화 과정이 수행할 필요 없이 사용 할 수 있다!
 * 
 * SplitPosition sPos = new SplitPosition(); 와 같은 코드를 사용할 필요가 없다.
 * 
 * Static 클래스는 프로젝트가 run(시작)되는 순간
 * 자동으로 객체를 1번 생성하여 method memory(기억영역)에 보관해 둔다.
 * 1번만 생성되기 때문에 메모리 공간을 1번만 차지하는 효과를 내고
 * 프로젝트의 어떠한 클래스, method 에서 든지 직접 접근하여 사용할 수 있다.
 */

public class SplitPosition {
	// static으로 선언된 변수는  누구나 어디에서든
	// 접근하여 읽기, 쓰기가 가능하다
	// 만약 WORD_ENG변수를 어딘가에서 0이아닌 다른값으로 변경을 해버리면
	// WORD_ENG 변수를 진짜 사용해야할 곳에서 문제가 발생할 수 있다.
