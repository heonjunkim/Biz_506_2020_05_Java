...
public class StudentServiceImplV1 implements StudentService {
	...
	// 필드변수를 public으로 선언해 놓으면 외부에서 필드변수에 직접 접근하여 값을 변형 시킬 수 있다.
	// 그러하면 예기치 못할 결과를 얻을 수 있다.
	...
	// 상속받은 클래스에서 현재 클래스의 필드변수를 같이 상속 받아서 사용하고자 할때는
	// 변수를 protected로 선언한다.
	
	// 현재 클래스에서는 private처럼 작동하고 
	// 상속받은 클래스에서도 마치 자신이 변수를 선언한것처럼 사용할 수 있게 된다.
	...
	public boolean input() {
		...
		// 데이터를 사용할지 말지를 결정하고
		// 입력하는 사용자에게 통보하고 다른 행동을 취할 수 있도록 하는것
		// 유효성 검사는 개발자입장에선 매우 귀찮은 단순노동일 경우가 많지만
		// 사용자입장에서는 유효성 검사가 많으면 그만큼 잘못된 데이터를 입력하는 경우를
		// 방지 할수 있다.
		// trade off < 사용자가 편리할수 있도록 찾는것
		try {
			intgrade = Integer.valueOf(strGrade);
		} catch (Exception e) {
			System.out.println("학년은 숫자만");
			System.out.println(studentVO.getName() + "학생정보를 다시 입력해 주세요");
			return true;
		}
		if(intgrade < 1|| intgrade > 4) {
			System.out.println("학년은1~ 4 까지만 가능");
			System.out.println(studentVO.getName() + "학생정보를 다시 입력해 주세요");
			return true;
		}
	...
	public void studentList() {
		...
		System.out.println("======================================");
		System.out.println("재학생 목록");
		System.out.println("학생번호\t이름\t학번\t전공");
		System.out.println("======================================");
